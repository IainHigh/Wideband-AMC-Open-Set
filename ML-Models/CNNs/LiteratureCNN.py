...
class ModulationClassifier(nn.Module):
    def __init__(self, num_classes, input_len=1024):
        ...
        self.block2_layers = nn.ModuleList([
            self._create_block2(input_channels=32 if i == 0 else 96, output_channels=96)
            for i in range(4)
        ])
    ...
    def _create_block2(self, input_channels, output_channels):
        ...
        return nn.ModuleDict({
            ...
            "branch2": nn.Sequential(
                nn.Conv1d(input_channels, 32, kernel_size=3, stride=2, padding=1),  # 32x(1x3)
                nn.BatchNorm1d(32),
                nn.ReLU()
            ),
            ...
            "residual": nn.Sequential(
                nn.Conv1d(input_channels, output_channels, kernel_size=1, stride=2),  # Residual path
    ...
    def forward(self, x):
        ...
        for block in self.block2_layers:
            ...
            branch_outputs = [block["branch1"](x), block["branch2"](x), block["branch3"](x)]
